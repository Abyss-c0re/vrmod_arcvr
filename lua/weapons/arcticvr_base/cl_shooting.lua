function SWEP:VR_PrimaryAttack()
    ...
    if self.OpenBolt then
        if self.Firemode == 0 then return end
        if not self.SlideLockedBack then return end
        self.SlideLockedBack = false
        self.SlideReleasing = true
        if self.LoadedRounds <= 0 then self:PlayNetworkedSound(nil, "SlideReleaseSound") end
        return
    end
    ...
    if self.VolleyFire then self:VolleyFireShoot() end
...
function SWEP:IsStabilizing()
    ...
    if self.ForegripGrabbed then
        dostab = true
    elseif (not self.TwoHanded or self.PistolStabilize) and (lhpos - rhpos):Length() <= 6 then
        dostab = true
    end
    return dostab
end
...
function SWEP:GetCurrentBarrel()
    ...
    if not self.VolleyFire then return 1 end
    if self.VolleyFireFromOneBarrel then return 1 end
    local vm = g_VR.viewModel
    local a = self.VolleyFireIndex
    if not self.VolleyFireBarrelToChamber then a = self.VolleyFireBarrelIndex end
...
function SWEP:DoEffects()
    local vm = g_VR.viewModel
    if not self.DisableMuzzleEffect then
        ...
        if muzzleeffect == "" then muzzleeffect = "CS_MuzzleFlash" end
        muzzleeffect = self:GetAttOverride("MuzzleEffectOverride") or muzzleeffect
        util.Effect(muzzleeffect, fx)
    end

    if not self.NonAutoloading and self.CaseEffect then
...
function SWEP:DoRecoil()
    ...
    if self:IsStabilizing() then
        ...
        if self.ForegripGrabbed then m = 0.75 * self:GetBuff("Buff_Recoil_TwoHand") end
    end
...
function SWEP:VolleyFireAdvance()
    ...
    if self.VolleyFireIndex > self.InternalMagazineCapacity + 0.5 then self.VolleyFireIndex = 1 end
    if not self.VolleyFireBarrelToChamber then
        self.VolleyFireBarrelIndex = self.VolleyFireBarrelIndex + 1
        if self.VolleyFireBarrelIndex > self.VolleyFireBarrels then self.VolleyFireBarrelIndex = 1 end
    end
end
...
function SWEP:DryFire()
    ...
    self.NeedAnotherTriggerPull = true
    if self.VolleyFire and self.VolleyFireAlwaysAdvance then self:VolleyFireAdvance() end
...
function SWEP:VolleyFireShoot()
    ...
    if self.VolleyFireChambers[a] ~= 2 then
        self:DryFire()
        return
    end
...
function SWEP:VR_OpenBoltShoot()
    ...
    if self.BeltFed then
        local magtbl = ArcticVR.MagazineTable[self.Magazine]
        if magtbl.IsBeltBox then if self.BeltAmountIn < self.BeltBullets then return end end
    end
...
function SWEP:VR_Shoot()
    ...
    if self.ForegripGrabbed then VRMOD_TriggerHaptic("vibration_left", 0, 0.3, 15, 20) end
    ...
    if not self.NonAutoloading then
        ...
        self:Cycle()
        if self.CanLockBack and self.Chambered == 0 and (self.Magazine or self.InternalMagazine) then self.SlideLockedBack = true end
