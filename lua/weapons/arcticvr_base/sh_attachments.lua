if CLIENT then
    function SWEP:AttRender()
        for i, k in pairs(self.Attachments) do
            ...
            if atttbl.Holosight then self:DrawHolosight(atttbl, k.CSModel, k.HSPiece, true) end
            if atttbl.LaserSight then self:DrawLaserSight(k.CSModel, atttbl) end
            if atttbl.AttThink then atttbl:AttThink(self, g_VR.viewModel, k.CSModel, k) end
        end
    end
    ...
    function SWEP:AttachmentBehaviour(pickup)
        ...
        if not leftent and not pickup then return end
        if leftent and not pickup then if not leftent.ArcticVRAttachment then return end end
        ...
        for i, k in pairs(self.Attachments) do
            ...
            if pickup then
                ...
                if not self:PositionInMaxs(lhpos, pos, mins, maxs) then continue end
                ...
                if not leftent.ArcticVRAttachment then return end
                local attname = leftent.AttID
                local atttbl = ArcticVR.AttachmentTable[attname]
                if atttbl.Bone then if k.Bone ~= atttbl.Bone then continue end end
                if k.Slot ~= atttbl.Slot then continue end
                if not self:CheckFlags(i, attname) then continue end
    ...
    function SWEP:RebuildAttModels()
        ...
        for i, k in pairs(self.Attachments) do
            ...
            local parent = vm
            if k.ParentToSlot then if self.Attachments[k.ParentToSlot].Installed and IsValid(self.Attachments[k.ParentToSlot].CSModel) then parent = self.Attachments[k.ParentToSlot].CSModel end end
...
function SWEP:CheckFlags(slotid, attname)
    ...
    for i, k in pairs(self.Attachments) do
        if not k.Installed then continue end
        if k.GivesFlags then table.Add(activeflags, k.GivesFlags) end
        local atttbl = ArcticVR.AttachmentTable[k.Installed]
        if atttbl.GivesFlags then table.Add(activeflags, atttbl.GivesFlags) end
    end
    ...
    if slottbl.WhitelistFlags then
        for i, k in pairs(slottbl.WhitelistFlags or {}) do
            if not table.HasValue(activeflags, k) then return false end
        end
    end
    return true
end
...
function SWEP:GetAttOverride(stat)
    ...
    for i, k in pairs(self.Attachments) do
        ...
        if atttbl[stat] then
            local precedence = 1
            if atttbl[stat .. "_Precedence"] then precedence = attbl[stat .. "_Precedence"] end
...
function SWEP:GetBuff(stat)
    ...
    for i, k in pairs(self.Attachments) do
        if not k.Installed then continue end
        local atttbl = ArcticVR.AttachmentTable[k.Installed]
        if atttbl[stat] then buff = buff * atttbl[stat] end
    end

    if self.Magazine then
        local magtbl = ArcticVR.MagazineTable[self.Magazine]
        if magtbl[stat] then buff = buff * magtbl[stat] end
    end
    return buff
end
...
function SWEP:CalcLaserStuff()
    ...
    if self.AverageLaserColor.g > toplasercolor then toplasercolor = self.AverageLaserColor.g end
    if self.AverageLaserColor.b > toplasercolor then toplasercolor = self.AverageLaserColor.b end
...
function SWEP:Attach(attname)
    ...
    for i, k in pairs(self.Attachments) do
        ...
        if atttbl.AttachFunc then atttbl.AttachFunc(self) end
        k.Installed = attname
        if CLIENT then
...
function SWEP:Detach(slotid)
    ...
    if not k.Installed then return end
    local atttbl = ArcticVR.AttachmentTable[k.Installed]
    if atttbl.DetachFunc then atttbl.DetachFunc(self) end
