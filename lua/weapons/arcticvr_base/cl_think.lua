function SWEP:VRThink()
    ...
    if self.SlideGrabbed or (self.PumpAction and self.ForegripGrabbed) then
        ...
        if !(self.PumpAction and self.ForegripGrabbed) then
            ...
            if !self:LeftHandInMaxs(slidebone, self.SlideMins * 2, self.SlideMaxs * 2) then
    ...
    for i, k in pairs(self.CaseBones) do
        ...
        if self.VolleyFire then
            ...
            if self.EmptyChambered >= i or self.Chambered >= i then
                vm:ManipulateBoneScale(self.BoneIndices[k], vec1)
            end
        end
    end
  ...
  if (self.LoadedBulletBone) then
        vm:ManipulateBoneScale(self.BoneIndices[self.LoadedBulletBone], ((self.LoadedRounds > 0) and vec1 or vec0))
    end
