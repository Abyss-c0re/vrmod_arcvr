...
function SWEP:VRThink()
    ...
    if g_VR.input.boolean_primaryfire or g_VR.input.boolean_turret then
        if not (self.Firemode == 1 or self.NeedAnotherTriggerPull) then if not self.TwoStageTrigger then self:VR_PrimaryAttack() end end
    else
        self.BurstLength = 0
    end
    ...
    if self.NonReciprocatingChargingHandle then
        ...
        if not self.CHandleBone then
            vm:ManipulateBonePosition(bone, extraoffset + self.CHandlePos * self.SlideDir)
            vm:ManipulateBoneAngles(bone, extraangle)
        end

        if self.BoneIndices.slide then vm:ManipulateBonePosition(self.BoneIndices.slide, self.SlidePos * self.SlideDir) end
    else
    ...
    if self.SlideGrabbed or self.PumpAction and self.ForegripGrabbed then
        ...
        if not slidebone then slidebone = self.BoneIndices.chandle end
        if self.PumpAction and self.ForegripGrabbed and self.PumpSlideOffset then pullback = self.PumpSlideOffset - coffset end
        if self.PumpAction and self.ForegripGrabbed and self.SlidePos == 0 and (self.Chambered > 0 or self.LoadedRounds > 0 and (self.ShootStraightFromMag or self.OpenBolt)) then
            pullback = 0
        ...
        if self.ReverseSlide then pullback = -pullback end
        if self.CHandleRaiseAtStart and self.CHandleRaisePos < self.CHandleRaiseAmount then pullback = 0 end
        ...
        if self.NonReciprocatingChargingHandle then self.CHandlePos = pullback end
        if self.ChargingHandlePullAmount then
        ...
        if self.LastSlidePos ~= self.SlidePos and self.SlidePos == self.SlideBlowbackAmount and not self.SlideReleasing then
            if self.OpenBolt then
                ...
                if self.CanDirectChamber then
                    if not self.SlideLockedBack then
                        self:Cycle(true)
                        net.Start("avr_rack")
                        net.SendToServer()
                    end
                elseif not self.ShootStraightFromMag then
                    self:Cycle(true)
                    net.Start("avr_rack")
                    net.SendToServer()
                end

                if self.SlideLockedBack and (self.LoadedRounds > 0 or not self.Magazine) then self.SlideLockedBack = false end
                self.SlideReleasing = true
            end

            if self.MagEjectOnOpen and self.MagCanDropFree and self.LoadedRounds <= 0 then self:EjectMagazine() end
        end
        ...
        if self.CHandleRaise and self.SlidePos >= self.SlideLockbackAmount or self.CHandleRaiseAtStart and self.SlidePos == 0 then
            ...
            if self.CHandleRaiseAtStart and self.SlidePos > 0 then self.CHandleRaisePos = self.CHandleRaiseAmount end
            if self.CHandleRaisePos == self.CHandleRaiseAmount and self.LastCHandleRaisePos ~= self.CHandleRaisePos then self:PlayNetworkedSound(nil, "BoltUpSound") end
        end

        self.LastCHandleRaisePos = self.CHandleRaisePos
        if not (self.PumpAction and self.ForegripGrabbed) then
            if self.BoneIndices.chandle then slidebone = self.BoneIndices.chandle end
            if not self:LeftHandInMaxs(slidebone, self.SlideMins * 2, self.SlideMaxs * 2) then self.SlideGrabbed = false end
        end
    else
        if not self.SlideNoAutoReciprocate then
            ...
            if self.NonReciprocatingChargingHandle then self.CHandlePos = math.Approach(self.CHandlePos, 0, FrameTime() * self.SlideBlowbackAmount * 500 / 60) end
        end
    end

    if self.SlideLockedBack or self.CHandleRaise and self.CHandleRaisePos > 0 then
        self.SlidePos = math.Clamp(self.SlidePos, self.SlideLockbackAmount, self.SlideBlowbackAmount)
        if self.NonReciprocatingChargingHandle and self.CHandleRaise and self.CHandleRaisePos > 0 then self.CHandlePos = math.Clamp(self.CHandlePos, self.SlideLockbackAmount, self.SlideBlowbackAmount) end
    end

    if self.SafetyBlocksSlide and self.Firemode == 0 then self.SlidePos = math.Clamp(self.SlidePos, 0, self.SlideLockbackAmount) end
    ...
    for i, k in pairs(self.FullBackOffset or {}) do
        if not self.BoneIndices[i] then continue end
    ...
    if self.LastSlidePos ~= self.SlidePos and self.SlidePos == 0 and self.SlideReleasing then
        ...
        if self.NonAutoloading and self.Chambered <= 0 and not self.ShootStraightFromMag then
            self:Cycle(true)
            net.Start("avr_rack")
            net.SendToServer()
        end
    ...
    if self.SlidePos == self.SlideBlowbackAmount then self.HammerDown = true end
    self.LastSlidePos = self.SlidePos
    if self.HKSlap and self.CHandleRaisePos > 0 and not self.SlideGrabbed then
        local slidebone = self.BoneIndices.slide
        if self.NonReciprocatingChargingHandle then slidebone = self.BoneIndices.chandle end
        if self:LeftHandInMaxs(slidebone, self.SlideMins, self.SlideMaxs) then
            ...
            if vel:Length() > 2.5 then
                ...
                if self.Chambered <= 0 then self:Cycle() end
            end
        end
    end

    self.RecoilBlowback = math.Approach(self.RecoilBlowback, 0, FrameTime() * 8 * self.RecoilBlowback)
    local ra_p = self.RecoilAngles[1]
    local ra_y = self.RecoilAngles[2]
    ra_p = math.ApproachAngle(ra_p, 0, FrameTime() * 4 * ra_p)
    ra_y = math.ApproachAngle(ra_y, 0, FrameTime() * 4 * ra_y)
    ...
    if self.LoadedRounds == 0 then self.BeltAmountIn = 0 end
    ...
    if self.DustCover then vm:ManipulateBoneAngles(self.BoneIndices.dustcover, Angle(0, 0, self.DustCoverPos)) end
    ...
    for i, k in pairs(self.CaseBones) do
        if not self.BoneIndices[k] then continue end
    ...
    for i, k in pairs(self.CaseBones) do
        ...
        if self.VolleyFire then
            if self.VolleyFireChambers[i] >= 1 then vm:ManipulateBoneScale(self.BoneIndices[k], vec1) end
        else
            if self.EmptyChambered >= i or self.Chambered >= i then vm:ManipulateBoneScale(self.BoneIndices[k], vec1) end
        end
    end
    ...
    for i, k in pairs(self.BulletBones) do
        if not self.BoneIndices[k] then continue end
        if self.VolleyFire then
            if self.VolleyFireChambers[i] == 2 then vm:ManipulateBoneScale(self.BoneIndices[k], vec1) end
        else
            if self.Chambered >= i then vm:ManipulateBoneScale(self.BoneIndices[k], vec1) end
            if self.OpenBolt then if self.LoadedRounds >= i then vm:ManipulateBoneScale(self.BoneIndices[k], vec1) end end
        end
    end

    if self.LoadedBulletBone then vm:ManipulateBoneScale(self.BoneIndices[self.LoadedBulletBone], self.LoadedRounds > 0 and vec1 or vec0) end
    if self.BreakAction and self.BreakActionChamberOpen then if self:MinimumVel(self.BreakActionCloseVector) then self:CloseChambers() end end
    ...
    if self.VolleyFire and not self.VolleyFireAutoEject then
    ...
    --     end
    -- end
    if g_VR.heldEntityLeft then ArcticVR.lasthelditem = g_VR.heldEntityLeft end
    ...
    for i, k in pairs(self.TargetMiscLerps) do
        if i == "BaseClass" then continue end
        if not self.MiscLerps[i] then continue end
        local rotspeed = 360
        if k.rotspeed then rotspeed = k.rotspeed end
        ...
        if i == self.BoneIndices.pivot and self.VolleyFireAutoEject and self.MiscLerps[i].ang == k.ang and self.MiscLerps[i].pos == k.pos then self:VolleyFireEject(self.VolleyFireAutoEjectUnspent) end
    end

    self:AttachmentBehaviour(false)
    vm:SetBodyGroups(self.DefaultBodygroups)
    for i, k in pairs(self.Attachments) do
        if not k.InstalledBG then continue end
        if k.Installed then
