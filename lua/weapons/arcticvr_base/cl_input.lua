...
function SWEP:VRInput(action, state)
    ...
    if action == "boolean_left_pickup" and state then
        ...
        if slidebone then
            if self:LeftHandInMaxs(slidebone, self.SlideMins * cv_slideplus, self.SlideMaxs * cv_slideplus) then
                ...
                if self.CHandleRaise or self.CHandleRaiseAtStart then self.SlideGrabVerticalOffset = vm:WorldToLocal(g_VR.tracking.pose_lefthand.pos).z + self.CHandleRaisePos end
    ...
    if action == "boolean_left_pickup" and state then
        ...
        local magtbl = nil
        if self.Magazine then magtbl = ArcticVR.MagazineTable[self.Magazine] end
        ...
        if magbone then
            if self:LeftHandInMaxs(magbone, self.MagazineInsertMins, self.MagazineInsertMaxs) then
                if not self.MagEjectOnOpen or self.SlidePos > self.SlideLockbackAmount then
    ...
    if action == "boolean_left_pickup" and state then
        if self.BoneIndices.ejector then
            if self:LeftHandInMaxs(self.BoneIndices.ejector, Vector(-1.5, -1.5, -1.5), Vector(1.5, 1.5, -1.5)) then
                ...
                self:BoneTap(self.BoneIndices.ejector, {
                    pos = self.EjectorTapOffset,
                    ang = Angle(0, 0, 0)
                }, 0.5)
                return
            end
    ...
    if self.TwoHanded then
        ...
        if GetConVar("arcticvr_grip_alternative_mode"):GetBool() then
            if self.ForegripGrabbed then
                if action == gripkey and not state then
                    self:UngripForegrip()
                    return
                end
    ...
    --Anniversaryzone
    if action == "boolean_reload" and state then
    ...
    if LocalPlayer():LookupBone(pouchbone) and LocalPlayer():GetBoneMatrix(LocalPlayer():LookupBone(pouchbone)) then
        pouch = LocalToWorld(Vector(3, 3, 0), Angle(0, 0, 0), LocalPlayer():GetBoneMatrix(LocalPlayer():LookupBone(pouchbone)):GetTranslation(), Angle(0, g_VR.characterYaw, 0))
        --Anniversaryzone
    else
    ...
    if g_VR.tracking.pose_lefthand.pos:DistToSqr(pouch) < dist * dist and not self.ForegripGrabbed then
        if action == "boolean_left_pickup" and state then
            ...
            if LocalPlayer():GetAmmoCount(self.Primary.Ammo) > 0 then
                ...
                if not self.NotAGun then surface.PlaySound("items/medshotno1.wav") end
            end

            self.NextCanSpawnMagTime = CurTime() + 0.1
            return
        elseif action == "boolean_left_pickup" and not state and IsValid(ArcticVR.lasthelditem) then
