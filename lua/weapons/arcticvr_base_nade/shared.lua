...
SWEP.NetworkedSounds = {"ThrowSound", "PinOutSound", "SpoonOffSound", "PinGrabbedSound"}
SWEP.ThrowSound = ""
...
if CLIENT then
		...
		if cv_pinsys:GetBool() then
			...
			if self.GrenadeGrabbed and self.BoneIndices.pin then
				if g_VR.input.boolean_left_pickup and self:LeftHandInMaxs(self.BoneIndices.pin, self.PinMins, self.PinMaxs) then
					...
					self.PinOffset = vm:WorldToLocal(g_VR.tracking.pose_lefthand.pos)[self.PinDirIn]
					return
				elseif not g_VR.input.boolean_left_pickup then
					self.PinGrabbed = false
					return
				end
			end
	...
	function SWEP:VRThink()
		...
		local vec0 = vec1 * 0
		local vecv = vec0
		if self.GrenadeGrabbed then vecv = vec1 end
		...
		if self.PinOut then vm:ManipulateBoneScale(self.BoneIndices.pin, vec0) end
	end
	...
	function SWEP:VR_Throw(src, ang, vel, angvel)
		...
		if not rocket:IsValid() then
			print("!!! INVALID ROUND " .. self.ShootEntity)
			return false
		end
