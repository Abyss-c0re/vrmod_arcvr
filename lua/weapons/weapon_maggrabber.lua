...
SWEP.TPEnts = {'player', 'prop_physics', 'prop_combine_ball', 'npc_grenade_frag', 'crossbow_bolt', 'npc_rollermine', 'npc_cscanner', 'npc_clawscanner', 'npc_manhack', 'portal_energy_pelet', 'npc_turret_floor', 'prop_vehicle_prisoner_pod',}
local pickable = {'models/props/metal_box.mdl', 'models/props/futbol.mdl', 'models/props/sphere.mdl', 'models/props/metal_box_fx_fizzler.mdl', 'models/props/turret_01.mdl', 'models/props/reflection_cube.mdl', 'npc_turret_floor', 'npc_manhack', 'models/props/radio_reference.mdl', 'models/props/security_camera.mdl', 'models/props/security_camera_prop_reference.mdl', 'models/props_bts/bts_chair.mdl', 'models/props_bts/bts_clipboard.mdl', 'models/props_underground/underground_weighted_cube.mdl', 'models/XQM/panel360.mdl'}
SWEP.BumpProps = {'models/props/portal_emitter.mdl'}
SWEP.BadSurfaces = {'prop_dynamic', 'prop_static', 'func_door', 'func_button', 'func_door_rotating', 'portal2_emancipationgrid'}
if SERVER then
...
hook.Add('AllowPlayerPickup', 'DisallowPickup', function(ply, ent) if IsValid(ply:GetActiveWeapon()) and IsValid(ent) and ply:GetActiveWeapon():GetClass() == 'weapon_portalgun' and table.HasValue(pickable, ent:GetModel()) or table.HasValue(pickable, ent:GetClass()) then return false end end)
...
function SWEP:Initialize()
	...
	if CLIENT then self:EmitSound('') end
end
...
local function PortalTrace(ent)
	if IsValid(ent) then
		if ent:IsPlayer() then -- players
			return false
		end

		if ent:IsWeapon() then return false end
		if table.HasValue(pickable, ent:GetModel()) or table.HasValue(pickable, ent:GetClass()) then -- some props
			return false
		end
	end
	return true
...
function SWEP:PickupProp(ent)
	if true then --[[table.HasValue( pickable, ent:GetModel() ) or table.HasValue( pickable, ent:GetClass() )]]
		self.HoldenProp = ent
...
function SWEP:Think()
	if self.Owner then
		...
		if self.Owner:KeyDown(IN_USE) and self.NextAllowedPickup < CurTime() and SERVER then
			...
			-- DROP FUNC
			if IsValid(self.HoldenProp) and self.HoldenProp ~= NULL then if self:DropProp() then return end end
			-- PICKUP FUNC
			if IsValid(tr.Entity) then
				local entsize = (tr.Entity:OBBMaxs() - tr.Entity:OBBMins()):Length() / 2
				if entsize > 45 then return end
				if not IsValid(self.HoldenProp) and tr.Entity:GetMoveType() ~= 2 then if self:PickupProp(tr.Entity) then return end end
			end
...
function SWEP:FirePortal(type)
	...
	if SERVER then
		...
		if not ownerent:IsWorld() then ent:SetParent(ownerent) end
