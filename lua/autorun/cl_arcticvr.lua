...
if CLIENT then
    ...
    net.Receive("avr_magin_forclient", function(len, ply)
        ...
        if wpn.ArcticVR then wpn.LoadedRounds = loaded end
    end)
    ...
    hook.Add("VRMod_PreRender", "avr_gunthink", function()
        local wpn = LocalPlayer():GetActiveWeapon()
        if ArcticVR.LastGun and ArcticVR.LastGun ~= wpn and ArcticVR.LastGun.ArcticVR then if IsValid(ArcticVR.LastGun) then ArcticVR.LastGun:VRHolster() end end
        if wpn.ArcticVR or wpn.ArcticVRNade then
            if ArcticVR.LastGun ~= wpn then wpn:VRDeploy() end
            if wpn.RTScope then wpn:RTScopeFunc(true) end
            for i, k in pairs(wpn.Attachments or {}) do
                ...
                if atttbl.RTScope then wpn:DrawRTScope(atttbl, k.CSModel, k.RTScopeMat, k.RTScopeSurface, true) end
            end
    ...
    hook.Add("VRMod_PreRenderRight", "avr_rightrender", function()
        local wpn = LocalPlayer():GetActiveWeapon()
        if wpn.ArcticVR or wpn.ArcticVRNade then
            if wpn.RTScope then wpn:RTScopeFunc(false) end
            for i, k in pairs(wpn.Attachments or {}) do
                if not k.Installed then continue end
                local atttbl = ArcticVR.AttachmentTable[k.Installed]
                if atttbl.RTScope then wpn:DrawRTScope(atttbl, k.CSModel, k.RTScopeMat, k.RTScopeSurface, false) end
            end
        end
    end)
    ...
    hook.Add("VRMod_Tracking", "avr_guntracking", function()
        ...
        if wpn.CenterForegrip then fg_offset[2] = 0 end
        ...
        if not dostab then
            ...
            if sf > 1 then dostab = true end
        end

        if sf > 1 and dostab then
            if not prevstab then ArcticVR.StabilityFrames = {} end
            ...
            if ArcticVR.StabilityFrameIndex > sf then ArcticVR.StabilityFrameIndex = 1 end
            ...
            for i = 1, #ArcticVR.StabilityFrames do
                ...
                if not sframe then continue end
                if i > sf then
                    ArcticVR.StabilityFrames[i] = nil
                    continue
                end

                if twohand then mean_lhpos = mean_lhpos + sframe.lhpos end
        ...
        if not wpn.ForegripGrabbed then
            if twohand then
        ...
        if wpn.ReverseForegrip then newang = -newang end
        newang = newang:Angle()
        newang[3] = origroll
