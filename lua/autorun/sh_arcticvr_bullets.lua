...
function ArcticVR:ShootPhysicalBullet(pos, ang, vel, appearance, mindamage, maxdamage, maxrange, inflictor, attacker, penleft, callback, disttravelled, lifetime)
   ...
   if bit.band(util.PointContents(pos), CONTENTS_WATER) == CONTENTS_WATER then bullet.underwater = true end
   --print(lifetime)
   --print(disttravelled)
...
function ArcticVR:ProcessPhysicalBullets()
   ...
   for i, bullet in pairs(ArcticVR.PhysBullets) do
      ...
      if bullet.visualonly then
         ...
         if bullet.lifetime >= ArcticVR.BulletLifeTime then table.remove(ArcticVR.PhysBullets, i) end
         continue
      end
      ...
      if CLIENT then filter = LocalPlayer() end
      if bullet.attacker and bullet.attacker:IsPlayer() then bullet.attacker:LagCompensation(true) end
      ...
      if bit.band(util.PointContents(newpos), CONTENTS_WATER) == CONTENTS_WATER and not bullet.underwater then
      ...
      elseif btr.Hit then
         ...
         if SERVER then
            ...
            if not IsValid(bullet.attacker) then return end
            --print(dmg)
            bullet.attacker:FireBullets({
               ...
               Callback = function(battacker, tr, dmginfo)
                  if bullet.callback then bullet.callback(battacker, tr, dmginfo) end
...
hook.Add("PostCleanupMap", "ArcticVR.CleanUpMap", function() ArcticVR.PhysBullets = {} end)