...
if SERVER then
    ...
    hook.Add("OnEntityCreated", "ArcticVR_NormalizeDefaultAmmo", function(ent)
        ...
        if ent.Primary.ClipSize <= 0 then ent.Primary.DefaultClip = clips end
    end)
    ...
    util.AddNetworkString("avr_meleeattack_leftfoot")
    util.AddNetworkString("avr_meleeattack_rightfoot")
    ...
    net.Receive("avr_rack", function(len, ply)
        local wpn = ply:GetActiveWeapon()
        if wpn.ArcticVR then wpn:Cycle() end
    end)
    ...
    net.Receive("avr_shoot", function(len, ply)
        ...
        local wpn = ply:GetActiveWeapon()
        if wpn.ArcticVR then wpn:VR_Shoot(src, ang) end
    end)
    ...
    net.Receive("avr_secondaryattack", function(len, ply)
        ...
        local ang = net.ReadAngle()
        local wpn = ply:GetActiveWeapon()
        if wpn.ArcticVR then wpn:VR_SecondaryShoot(src, ang) end
    end)
    ...
    net.Receive("avr_meleeattack", function(len, ply)
        ...
        local wpn = ply:GetActiveWeapon()
        if wpn.ArcticVR then wpn:VR_Melee(src, vel) end
    end)
    ...
    net.Receive("avr_meleeattack_leftfoot", function(len, ply)
        local src = Vector(0, 0, 0)
        src[1] = net.ReadFloat()
        src[2] = net.ReadFloat()
        src[3] = net.ReadFloat()
    ...
    net.Receive("avr_meleeattack_rightfoot", function(len, ply)
        local src = Vector(0, 0, 0)
        src[1] = net.ReadFloat()
        src[2] = net.ReadFloat()
        src[3] = net.ReadFloat()
    ...
    net.Receive("avr_nadethrow", function(len, ply)
        ...
        local wpn = ply:GetActiveWeapon()
        if wpn.ArcticVRNade then wpn:VR_Throw(src, ang, vel, angvel) end
    end)
    ...
    local function GrabAndPose(ent, pos, ang, lefthand, ply)
        ...
        for k2, v2 in pairs(g_VR[ply:SteamID()].heldItems) do
            if v2.ent == ent then
                table.remove(g_VR[ply:SteamID()].heldItems, k2)
                found = true
            end
        end
        ...
        g_VR[ply:SteamID()].heldItems[#g_VR[ply:SteamID()].heldItems + 1] = {
            ent = ent,
            left = lefthand,
            localPos = locpos,
            localAng = locang,
            targetPos = Vector(0, 0, 0),
            targetReached = SysTime()
        }
    ...
    function ArcticVR.CreateMag(magid, rounds)
        ...
        if not mag or not IsValid(mag) then
    ...
    net.Receive("avr_magout", function(len, ply)
        ...
        if grab then
            ...
            -- end
            timer.Simple(timertime, function() GrabAndPose(mag, hpos, hang, lefthand, ply) end)
        end
    end)
