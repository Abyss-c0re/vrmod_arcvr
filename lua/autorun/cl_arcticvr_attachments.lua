net.Receive("avr_sendatts", function(len, ply)
    ...
    for i, k in pairs(atts) do
        ...
        if not ent.Attachments[i] then continue end
        if atttbl.AttachFunc then atttbl.AttachFunc(ent) end
        ent.Attachments[i].Installed = attname
    end

    if ent == LocalPlayer():GetActiveWeapon() then ent:RebuildAttModels() end
end)
...
hook.Add("PostDrawTranslucentRenderables", "avr_drawplayerlasers", function(depth, sky)
    ...
    for i, k in pairs(player.GetAll()) do
        ...
        if not wpn then continue end
        if not IsValid(wpn) then continue end
        if not wpn.ArcticVR then continue end
        if wpn.TotalLaserStrength == 0 then continue end
        if not wpn.AverageLaserColor then continue end
        local pos = k:EyePos()
