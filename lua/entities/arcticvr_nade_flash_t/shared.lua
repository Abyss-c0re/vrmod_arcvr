...
local function FlashExplosion(flashorigin)
    ...
    for _, k in pairs(targets) do
        if k:IsPlayer() then
            ...
            if k:VisibleVec(flashorigin) then k:ScreenFade(SCREENFADE.IN, Color(255, 255, 255, 255), 2.5, time) end
            k:SetDSP(37, false)
        elseif k:IsNPC() then
            k:SetNPCState(NPC_STATE_PLAYDEAD)
            if timer.Exists(k:EntIndex() .. "_avr_flashtimer") then timer.Remove(k:EntIndex() .. "_avr_flashtimer") end
